...
class ResearcherLoopGuardMiddleware(AgentMiddleware):
    ...
    turns indefinitely, and a worker stopped by anything other than its own finalization loses
    ...
    def _guard_model_request(self, request):
        ...
        ``ResearchNotes``, which ends the invocation. Below that, only the exhausted
        source/think tools go.
        ...
        here. Overshooting stops the worker a turn early, which is the safe direction - one
        turn of research is a far cheaper loss than the notes an unterminated worker discards.
        """
        state = CURRENT_RESEARCHER_GUARD_STATE.get()
        if not self._config.enabled or state is None:
            return request
