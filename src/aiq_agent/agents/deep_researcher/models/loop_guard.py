...
# Past a few hundred turns a per-worker turn ceiling has stopped being a breaker - a worker
# allowed that many turns is not one this guard can meaningfully bound - so the ceiling only has
# to keep a typo from disabling it outright.
...
class ResearcherLoopGuardConfig(BaseModel):
    ...
    enabled: bool = Field(
        ...
        description=(
            ...
            "the turn ceiling) and withdraws no tool. Only turn this off if the guard is "
            "truncating healthy runs and raising the individual limits did not help."
        ),
    )
    ...
    max_model_turns_per_query: int = Field(
        ...
        description=(
            ...
            "ResearchNotes. This is what closes the loop rather than narrowing it - the other "
            "limits leave the filesystem tools callable. The default covers the worst path the "
            "other limits still permit: 25 source calls, a think between each, plus "
            "orientation and synthesis turns. May not exceed "
