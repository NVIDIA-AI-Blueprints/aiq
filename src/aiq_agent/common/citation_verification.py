...
class SerializedCitationVerificationOutcome(TypedDict):
    """Serialized closed citation-verification disposition."""
...
_VALID_CITATION_VERIFICATION_PAIRS = {
    (VERIFIED_CITATION_STATUS, VALID_CITATIONS_REASON),
    (UNVERIFIED_CITATION_STATUS, NO_SOURCES_REASON),
    (UNVERIFIED_CITATION_STATUS, NO_VALID_CITATIONS_REASON),
    (UNVERIFIED_CITATION_STATUS, NO_TOOLS_AVAILABLE_REASON),
    (DISABLED_CITATION_STATUS, VERIFICATION_DISABLED_REASON),
}


def _validate_serialized_citation_verification_outcome(
    value: SerializedCitationVerificationOutcome,
) -> SerializedCitationVerificationOutcome:
    """Validate the status/reason relationship at Pydantic state boundaries."""
    if (value["status"], value["reason"]) not in _VALID_CITATION_VERIFICATION_PAIRS:
        raise ValueError("Invalid citation-verification outcome")
    return value


CitationVerificationOutcomeState = Annotated[
    SerializedCitationVerificationOutcome,
    AfterValidator(_validate_serialized_citation_verification_outcome),
]
...
class CitationVerificationOutcome:
    ...
    def __post_init__(self) -> None:
        """Reject contradictory status/reason pairs at construction time."""
        if (
            not isinstance(self.status, str)
            or not isinstance(self.reason, str)
            or (self.status, self.reason) not in _VALID_CITATION_VERIFICATION_PAIRS
        ):
            raise ValueError("Invalid citation-verification outcome")

    def to_dict(self) -> SerializedCitationVerificationOutcome:
...
def coerce_citation_verification_outcome(value: Any) -> CitationVerificationOutcome | None:
    ...
    if (
        ...
        or status not in _VALID_CITATION_VERIFICATION_STATUSES
        or reason not in _VALID_CITATION_VERIFICATION_REASONS
        or (status, reason) not in _VALID_CITATION_VERIFICATION_PAIRS
    ):
        return None
    return CitationVerificationOutcome(status=status, reason=reason)


def citation_verification_outcome_dict(value: Any) -> SerializedCitationVerificationOutcome | None:
...
def combine_citation_verification_outcomes(parent: Any, child: Any) -> SerializedCitationVerificationOutcome | None:
