...
class TestChatResearcherState:
    ...
    def test_state_validates_last_report_citation_verification_status(self):
        status = {"status": "unverified", "reason": "no_valid_citations"}

        state = ChatResearcherState(messages=[], last_report_citation_verification_status=status)

        assert state.last_report_citation_verification_status == status
        with pytest.raises(ValidationError, match="Invalid citation-verification outcome"):
            ChatResearcherState(
                messages=[],
                last_report_citation_verification_status={"status": "disabled", "reason": "valid_citations"},
            )
