...
@pytest.mark.asyncio
async def test_get_job_report_includes_public_citation_verification_status(job_report_app):
    """Test that the report response exposes only the closed public disposition."""
    from aiq_agent.agents.deep_researcher.models import citation_verification_warning

    app, job, job_store, _db_url = job_report_app
    job.output = {
        "report": "# Unverified Report",
        "citation_verification_status": {
            "status": "unverified",
            "reason": "no_sources",
            "unavailable_tools": ["internal-only diagnostic"],
        },
    }
    ...
    assert response.report == f"{warning}\n\n# Unverified Report"
    assert response.citation_verification_status.model_dump() == {
        "status": "unverified",
        "reason": "no_sources",
        "warning": warning,
    }
