...
PYTHON_BIN=""
DEFAULT_LANGCHAIN_NVIDIA_INSTALL_SPEC="langchain-nvidia-openshell"
...
Sets up OpenShell for AI-Q:
  ...
  4. Installs the langchain-nvidia OpenShell adapter.
...
install_openshell_python() {
    ...
    local adapter_install_args=("${editable_args[@]}")
...
Set LANGCHAIN_NVIDIA_REPO to a uv install spec or local checkout, then rerun.
Examples:
  LANGCHAIN_NVIDIA_REPO=langchain-nvidia-openshell scripts/setup_openshell.sh
  LANGCHAIN_NVIDIA_REPO='git+https://example.com/langchain-nvidia.git#subdirectory=libs/openshell' scripts/setup_openshell.sh
