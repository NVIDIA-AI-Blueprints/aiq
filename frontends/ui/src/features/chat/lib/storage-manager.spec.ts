...
describe('storage-manager', () => {
  ...
  describe('cleanupExpiredSessions', () => {
    ...
    test('deletes sessions older than 24h', () => {
      const conversations = [
        makeConversation('s_old', 'user1', isoHoursAgo(25)),
        makeConversation('s_fresh', 'user1', isoHoursAgo(10)),
      ]
      ...
      const stored = JSON.parse(localStorage.getItem('aiq-chat-store') || '{}')
      const remainingIds = stored.state?.conversations?.map((c: Conversation) => c.id) || []
      expect(remainingIds).toEqual(['s_fresh'])
    })

    test('keeps sessions younger than 24h', () => {
      const conversations = [
        makeConversation('s_a', 'user1', isoHoursAgo(1)),
        makeConversation('s_b', 'user1', isoHoursAgo(23)),
      ]
    ...
    test('protects expired sessions with active deep research', () => {
      const busyMessages: ChatMessage[] = [
        {
          id: 'msg1',
          role: 'assistant',
      ...
      const stored = JSON.parse(localStorage.getItem('aiq-chat-store') || '{}')
      const remainingIds = stored.state?.conversations?.map((c: Conversation) => c.id) || []
      expect(remainingIds).toEqual(['s_busy_old'])
    })

    // Production stores currentConversation as the ID string (see
    // prunePersistedChatState in store.ts), so these tests bypass the
    // shared setStoreData helper which writes the full object.
    ...
    test('clears currentConversation reference when current is deleted', () => {
      const conversations = [
        makeConversation('s_current_old', 'user1', isoHoursAgo(30)),
        makeConversation('s_other', 'user1', isoHoursAgo(2)),
      ]

      writePersistedState(conversations, 's_current_old')
    ...
    test('keeps currentConversation reference when current is fresh', () => {
      const conversations = [
        makeConversation('s_current_fresh', 'user1', isoHoursAgo(2)),
        makeConversation('s_old', 'user1', isoHoursAgo(30)),
      ]

      writePersistedState(conversations, 's_current_fresh')

      cleanupExpiredSessions()

      const stored = JSON.parse(localStorage.getItem('aiq-chat-store') || '{}')
      expect(stored.state?.currentConversation).toBe('s_current_fresh')
    })

    test('returns empty array and writes nothing when no candidates', () => {
      const conversations = [makeConversation('s_a', 'user1', isoHoursAgo(1))]
      setStoreData(conversations)
    ...
    test('returns empty array when chat store is missing', () => {
      const deleted = cleanupExpiredSessions()
      expect(deleted).toEqual([])
    })

    test('removes sessions across users (not scoped per-user)', () => {
      const conversations = [
        makeConversation('s_user1_old', 'user1', isoHoursAgo(48)),
        makeConversation('s_user2_old', 'user2', isoHoursAgo(48)),
        makeConversation('s_user1_fresh', 'user1', isoHoursAgo(2)),
      ]
      ...
      expect(deleted.sort()).toEqual(['s_user1_old', 's_user2_old'])
    })

    test('keeps sessions with malformed updatedAt (NaN) instead of deleting', () => {
      // Date constructor returns Invalid Date → getTime() === NaN. We must
      // never silently delete such sessions just because the comparison is
      // NaN-driven; they should round-trip untouched.
      ...
      const fresh = makeConversation('s_fresh', 'user1', isoHoursAgo(2))

      setStoreData([malformed, fresh])
      ...
      const stored = JSON.parse(localStorage.getItem('aiq-chat-store') || '{}')
      const remainingIds = (stored.state?.conversations ?? []).map((c: Conversation) => c.id)
      expect(remainingIds).toEqual(['s_malformed', 's_fresh'])
    })
  })
